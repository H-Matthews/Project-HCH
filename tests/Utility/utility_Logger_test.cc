...
class UtilityLoggerTest : public ::testing::Test {
  protected:
    ...
    void SetUp() override {
        ...
        textFileSink = std::make_shared<Utility::TextFileSink>("./testing_output",
                                                               "Utility_logger_test", ".log");
...
TEST(UtilityLoggerTextFactoryTest, textFileFactory) {
    auto textFileLogger = Utility::createTextFileLogger("textLogger", "./test", "file", ".log",
...
/*
    ...
    Description: Ensure the function addSinkList properly adds the sinks defined in the list to the
   Logger Expectation: logger_default should have two sinks colorConsoleSink, textFileSink
*/
...
/*
    Test: getFilePath
    Description: Ensure the function addSinkList properly returns the filePath associated with the
   textFileSink Expectation: TextFileSink->getFilePath should be equal to the value given to the
   constructor
*/
...
TEST_F(UtilityLoggerTest, swapFormatter_Test) {
    ...
    for (const auto& updatedSink : updatedSinks) {
        ...
        Utility::KeyValueFormatter* keyValueFormatter =
            dynamic_cast<Utility::KeyValueFormatter*>(currentFormatter);
