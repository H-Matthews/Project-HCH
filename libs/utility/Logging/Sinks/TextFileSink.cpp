...
Utility::TextFileSink::Builder::Builder() : mSink(std::make_shared<TextFileSink>()) {}

Utility::TextFileSink::Builder&
Utility::TextFileSink::Builder::outputDirectory(std::string outputDirectory) {
...
Utility::TextFileSink::Builder&
Utility::TextFileSink::Builder::logExtension(std::string logExtension) {
...
Utility::TextFileSink::Builder&
Utility::TextFileSink::Builder::formatter(std::unique_ptr<LogFormatter> f) {
...
Utility::TextFileSink::TextFileSink()
    : LogSink(SINK_IDENTIFIER), mOutputDirectory(""), mFileName(""), mLogExtension("") {}

Utility::TextFileSink::TextFileSink(const std::string& outputDirectory, const std::string& fileName,
                                    const std::string& logExtension, LogLevel level)
    : LogSink(SINK_IDENTIFIER, std::make_unique<KeyValueFormatter>(), level),
      mOutputDirectory(outputDirectory), mFileName(fileName), mLogExtension(logExtension) {
    openFile();
}
...
void Utility::TextFileSink::sinkData(std::string_view message, LogLevel level,
    ...
    if (mFormatter) {
        ...
        LogRegistry::instance()->getGlobalLogger()->logError("Logger: " + LogSink::mSinkIdentifier +
...
std::shared_ptr<Utility::Logger> Utility::createTextFileLogger(const std::string& loggerName,
                                                               ...
                                                               LogLevel level) {
    auto textFileSink =
        std::make_shared<Utility::TextFileSink>(outputDirectory, fileName, logExtension, level);
