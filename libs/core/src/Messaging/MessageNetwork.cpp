...
void Core::MessageNetwork::addSubscriberTopic(const std::string& nodeName, Messages::ID messageID)
{
    ...
    else
    {
        ...
        {
            if constexpr(Utility::CAN_LOG)
                mLogger->logInfo("Added Topic \"" + messageIDEnumToString(messageID) + "\" for Subscriber Node \"" + nodeName + "\"");
        }
    }
...
void Core::MessageNetwork::addPublisherTopic(const std::string& nodeName, Messages::ID messageID )
{
    ...
    else
    {
        ...
        {
            if constexpr(Utility::CAN_LOG)
                mLogger->logError("Node: \"" + nodeName + "\" is already a publisher for topic " + messageIDEnumToString(messageID));
        }
        else 
        {
            if constexpr(Utility::CAN_LOG)
                mLogger->logInfo("Added Topic \"" + messageIDEnumToString(messageID) + "\" for Publisher Node \"" + nodeName + "\"");
        }
    }
...
void Core::MessageNetwork::unRegisterSubscriberNode(const std::string& nodeName)
{
    ...
    {
        if constexpr(Utility::CAN_LOG)
            mLogger->logError("Subscriber Node \"" + nodeName + "\" is NOT registered as a Subscriber. Unregister atttempt failed");
    }
...
void Core::MessageNetwork::unRegisterPublisherNode(const std::string& nodeName)
{
    ...
    {
        if constexpr(Utility::CAN_LOG)
            mLogger->logError("Publisher Node \"" + nodeName + "\" is NOT registered as a Publisher. Unregister atttempt failed");
    }
...
void Core::MessageNetwork::addressPendingRequests()
{
    ...
    for (const auto& [hashValue, messageSet] : mPendingPublisherRequests)
    {
        ...
        for(auto messageSetIT = messageSet.begin(); messageSetIT != messageSet.end(); messageSetIT++)
        {
            ...
            {
                if constexpr(Utility::CAN_LOG)
                    mLogger->logInfo("Removing topic " + messageIDEnumToString((*messageIT)) + " from publisher node");
    ...
    for (const auto& [hashValue, messageSet] : mPendingSubscriberRequests)
    {
        ...
        for(auto messageSetIT = messageSet.begin(); messageSetIT != messageSet.end(); messageSetIT++)
        {
            ...
            {
                if constexpr(Utility::CAN_LOG)
                    mLogger->logInfo("Removing topic " + messageIDEnumToString((*messageIT)) + " from subscriber node");
