...
Core::TOMLConfigReader::readFile(const std::filesystem::path& filePath,
    ...
    // BEGIN PARSING TOML DATA TO CONFIG
    // TREE-----------------------------------------------------------
...
void Core::TOMLConfigReader::processTOMLData(const toml::node& tomlNode,
                                             std::shared_ptr<ConfigNode> configNode) {
    // Determine Node Type
...
void Core::TOMLConfigReader::processArrayTOMLData(const std::string& keyNode,
                                                  const toml::node& tomlNode,
                                                  std::shared_ptr<ConfigNode> configNode) {
    ...
    if (firstElement.is_string()) {
        ...
        for (const auto& elem : *tomlArray) {
            values.push_back(
                elem.as_floating_point()->value_or(std::numeric_limits<double>::max()));
        }
...
void Core::TOMLConfigReader::processPrimitiveTOMLData(const std::string& keyNode,
                                                      const toml::node& tomlNode,
                                                      std::shared_ptr<ConfigNode> configNode) {
    ...
    if (tomlNode.is_string()) {
        ...
        configNode->insertValuePair(
            keyNode, tomlNode.as_integer()->value_or(std::numeric_limits<int>::max()));
