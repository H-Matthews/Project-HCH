...
void Core::Configuration::initializeOutputDirectory()
{
    ...
    if (mRootSection)
    {
        auto configSection = mRootSection->getSection("Configuration");
        if (configSection)
            configuredOutDirectory = configSection->getString("out_directory");
    }
...
void Core::Configuration::initializeAssetsDirectory()
{
    ...
    if (mRootSection)
    {
        ...
        if (configSection)
        {
            configuredAssetDir = configSection->getString("asset_directory");
            configuredAssetFontDir = configSection->getString("asset_font_directory");
            configuredAssetTextureDir = configSection->getString("asset_texture_directory");
        }
    }
...
std::pair<bool, std::string> Core::Configuration::parseRootFile()
{
    ...
    auto filesSection = mRootSection->getSection("Configuration_Files");
    if (filesSection)
    {
        if (auto coreConfigFile = filesSection->getString("core_configurables"))
            mConfigFiles.push_back(mConfigDirPath + "/" + *coreConfigFile);

        if (auto prefabConfigFile = filesSection->getString("prefabs"))
