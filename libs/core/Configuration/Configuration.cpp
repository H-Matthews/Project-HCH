...
Core::ConfigSpec::ConfigSpec() : rootConfigFile(""), configDirectory(""), configReader(nullptr) {}

Core::Configuration::Configuration(ConfigSpec configSpec)
    : mProjectDirectory(PROJECT_DIR), mRootFile(std::move(configSpec.rootConfigFile)),
      mConfigDirectory(std::move(configSpec.configDirectory)),
      mConfigDirPath(mProjectDirectory + "/" + mConfigDirectory), mOutputDirPath(), mAssetDirPath(),
      mAssetFontsDirPath(), mAssetTexturesDirPath(),
      mConfigReader(std::move(configSpec.configReader)), mConfigFiles(), mDirectoryBits(0) {
    ...
    if (!(std::filesystem::is_directory(mConfigDirPath))) {
        ...
        throw ConfigurationException(
            std::string("Config directory could NOT be found" + mConfigDirPath).c_str());
    }

    if (mRootFile.empty()) {
        if constexpr (Utility::CAN_LOG)
            Utility::LogRegistry::instance()->getGlobalLogger()->logError(
                "Root file was NOT populated ");
...
void Core::Configuration::initializeOutputDirectory() {
    ...
    if (!(std::filesystem::create_directory(mOutputDirPath))) {
        ...
        std::string exceptionMessage =
            "APP_Directory: " + mOutputDirPath + " could NOT be created ";
        throw ConfigurationException(exceptionMessage.c_str());
    }
...
void Core::Configuration::initializeAssetsDirectory() {
    ...
    std::shared_ptr<ConfigNode> rootNode = ConfigurationTree::instance()->getRootNode();
    auto configuredAssetDir = ConfigUtils::findValueByNode<std::string>(
        rootNode, "root.Configuration", "asset_directory");

    auto configuredAssetFontDir = ConfigUtils::findValueByNode<std::string>(
        rootNode, "root.Configuration", "asset_font_directory");

    auto configuredAssetTextureDir = ConfigUtils::findValueByNode<std::string>(
        rootNode, "root.Configuration", "asset_texture_directory");
...
std::pair<bool, std::string> Core::Configuration::parseRootFile() {
    ...
    if constexpr (Utility::CAN_LOG)
        Utility::LogRegistry::instance()->getGlobalLogger()->logDebug("Parsing config file: " +
                                                                      rootFilePath);

    std::shared_ptr<ConfigNode> rootConfigNode = nullptr;
    try {
        auto retStatus =
            mConfigReader->readFile(std::filesystem::path(rootFilePath), rootConfigNode);
        ...
        auto coreConfigFile = ConfigUtils::findValueByNode<std::string>(
            rootNode, "root.Configuration_Files", "core_configurables");

        if (coreConfigFile)
            mConfigFiles.push_back(std::string(mConfigDirPath + "/" + *coreConfigFile));

        auto prefabConfigFile = ConfigUtils::findValueByNode<std::string>(
            rootNode, "root.Configuration_Files", "prefabs");

        if (prefabConfigFile)
            mConfigFiles.push_back(std::string(mConfigDirPath + "/" + *prefabConfigFile));
    } catch (const toml::parse_error& e) {
        return std::make_pair(false,
                              "TOML parse error in file: " + rootFilePath + " - " + e.what());
    }
...
std::pair<bool, std::string> Core::Configuration::parseConfigFiles() {
    for (const auto& configFile : mConfigFiles) {
        ...
        try {
            auto retStatus =
                mConfigReader->readFile(std::filesystem::path(configFile), fileConfigNode);
            ...
            return std::make_pair(false,
                                  "TOML parse error in file: " + configFile + " - " + e.what());
        }
    }
...
Core::Configuration::buildConfigFilePath(const std::string& configFile) {
